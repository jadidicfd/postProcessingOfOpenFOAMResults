function [bandwidth,density,X,Y]=mjKDE2d(data,n,MIN_XY,MAX_XY)
...
%   data=[randn(5000,2);
%       randn(5000,1), randn(5000,1);];
%   % call the routine
%     [bandwidth,density,X,Y]=mjKDE2d(data);
%   % plot the data and the density estimate
%     [C,h] = contour(X,Y,density,5); 
%     hold on
%     clabel(C,'manual','FontWeight','bold','Color','blue')
%     %plot(data(:,1),data(:,2),'r.','MarkerSize',5)
% %

% Example (simple Gaussian mixture 2)
% clear all
%   % generate a Gaussian mixture with distant modes
%   data=[randn(500,2);
